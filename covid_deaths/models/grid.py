...
def grid_search_orbit(
    ...
    **kwargs,
    ...
    def evaluate_params(tuned_param_dict):
        if verbose:
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(evaluate_params, param_dict) for param_dict in param_list_dict]
        for future in tqdm(concurrent.futures.as_completed(futures), total=len(futures)):
            metric_values.append(future.result())
